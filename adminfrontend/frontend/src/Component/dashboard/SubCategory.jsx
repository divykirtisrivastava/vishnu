...
console.log(data)
  ...
  const handleDelete = async (id) => {
    ...
    if(flag){
     await axios.delete(`https://actl.co.in/shop/subcategorydelete/${id}`);
      getProfile()
    }
  };
  ...
  return (
    ...
    <div className='absolute flex flex-col items-center w-[80%] left-[20%] top-20'>
      <Link to='/admin/newsubcategory' className='p-3 rounded-lg bg-sky-700 m-5 text-white font-bold hover:bg-black'>Add New Sub-Category</Link>
            ...
            <table className="min-w-full divide-y divide-gray-200 bg-white">
              <thead>
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Sub-Category Action </th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {data.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><img src={`https://actl.co.in/uploads/${item.subcategoryImage}`} alt="" className='h-20 w-20'/></td>
